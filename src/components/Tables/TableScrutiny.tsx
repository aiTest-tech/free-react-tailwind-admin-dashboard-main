...
const TableScrutiny = (props: RequestData) => {
  const [apiData, setApiData] = useState<any>(null); // To store the fetched scrutiny data
  const [isLoading, setIsLoading] = useState(true); // Loading state
  const [isError, setIsError] = useState(false); // Error state

  // Fetch the scrutiny data from the API
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('http://10.10.2.179:5555/api/analytics/');
        const data = await response.json();
        setApiData(data.l1_scrutiny); // Assuming the data is under `l1_scrutiny`
  ...
  return (
    <div className="rounded-2xl border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      ...
      <div className="flex flex-col">
        ...
        {isLoading
          ? renderSkeleton() // Show skeleton loader when loading
          : apiData?.map((brand: any, key: number) => (
              ...
              >
                ...
                <div className="flex items-center justify-center p-2.5 xl:p-5">
                  <p className="text-black dark:text-green-500 font-extrabold text-xl">
                    {brand.success_rate}%
                  </p>
                </div>
